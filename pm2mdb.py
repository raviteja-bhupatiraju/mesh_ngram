    ...
    for pmarticle in root.getchildren():
        ...
        if len(pub_year) != 4:
            ...
            if len(medline_date) == 9 and medline_date[4] == '-': # 1976-1977
                ...
                # TODO: 1994-94 | 1981, reçu | 1978-1979, reçu
                try:
        ...
        try:
            #pass
            db.test_int.insert_one({'_id': pmid, 'title': title, 'year': pub_year, 'month': pub_month, 'mesh': mesh})
        except mdb.errors.DuplicateKeyError:
            print('Duplicate: ', pmid)

files = [fn_templ % str(i).zfill(3) for i in range(1, 892 + 1)]
if __name__ == '__main__': mp.Pool(3).map(process_xml, files)
